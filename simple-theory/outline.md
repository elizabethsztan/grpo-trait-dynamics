...
- **Sampled (practical).** On a real model you never enumerate the action space; you only have finite rollouts. So we also estimate the same covariance from `price_samples` fresh draws $(x,a)$ with $x\sim\mathrm{Uniform}(\text{eval})$, $a\sim\pi_t(\cdot\mid x)$ — the Monte-Carlo estimator. Its error shrinks like $1/\sqrt{\texttt{price\_samples}}$, so sweeping the budget shows how many rollouts are needed to recover the selection term in practice; the exact line is the ground truth it converges to.

The draws must be **fresh** and use a diagnostic random stream independent of the actions used for the GRPO update. Reusing update rollouts biases the estimate upward because the update raised $\pi_{t+1}$ on exactly those sampled high-advantage actions. Both the neural and tabular policies report the sampled estimate; the neural policy evaluates on its held-out table, while the tabular policy samples from its finite $N\times K$ table.